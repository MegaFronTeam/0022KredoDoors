...
const JSCCommon = {
	...
	tabscostume(tab) {
		...
		$('.' + tab + '__caption').on('click', '.' + tab + '__btn:not(.active)', function (e) {
			...
			if ($(this).hasClass('cs_tab_disabled')) {

				if (_this.tippyInstance) {
					_this.tippyInstance.destroy();
				}

				let tabCode = $(this).data('code');
				if ($('#js_tab_warn_' + tabCode).length > 0) {
					_this.tippyInstance = tippy($(this).get(0), {
						theme: 'light',
						content: $('#js_tab_warn_' + tabCode).html()
					});
					_this.tippyInstance.show();
				}
...
function eventHandler() {
	...
	// }
	let block2 = document.querySelectorAll(".sBlockVar2");

	if(block2.length){
		for (const item of block2) {			
			new Swiper(item.querySelector('.sBlockVar2__slider--js'), {
				navigation: {
	...
	let block3 = document.querySelectorAll(".sBlockVar3");

	if(block3.length){
		for (const item of block3) {			
			new Swiper(item.querySelector('.sBlockVar3__slider--js'), {
				slidesPerView: 'auto',
				spaceBetween:30,
				navigation: {
					nextEl: item.querySelector('.swiper-button-next'),
	...
	$(document).on("click", '.link-modal-custom-js  ', function (e) {
		e.preventDefault();
		var x = this.hash;
		var listIten = this.dataset.modalScroll;
		document.body.classList.add("fixed2");
		$(x).addClass("active");

		if(!listIten) return;
		document.querySelector(listIten).scrollIntoView(true);
	})
